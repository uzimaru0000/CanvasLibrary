...
class Drowable extends Node {
  ...
  get vertex() {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      vert.push(
        new Vector(
          Math.cos(this.globalRotation) * v.x -
            Math.sin(this.globalRotation) * v.y,
          Math.sin(this.globalRotation) * v.x +
            Math.cos(this.globalRotation) * v.y
        ).add(this.globalPos)
      );
    }
    return vert;
  ...
  withIn(target, radius, callback) {
    if (
      target instanceof Drowable &&
      target.globalPos.sub(this.globalPos).length <= 2 * radius
    ) {
  ...
  isHit(target) {
    ...
    return target.vertex.some((_, n, v) =>
      this.vertex.some((_, m, w) => {
        ...
        return (
          v1.cross(v2) * v1.cross(v3) < 0 && v4.cross(v5) * v4.cross(v6) < 0
        );
      })
    );
  }
...
class AnimationSprite extends Drowable {
  ...
  __draw(display) {
    this._context.drawImage(
      this.textures[this.frame].image,
      0,
      0,
      this.width,
      this.height
